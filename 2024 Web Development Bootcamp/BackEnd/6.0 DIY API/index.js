...
app.get("/random", (req, res) => {
  const index = (Math.round(Math.random() * jokes.length));
...
app.get("/filter", (req, res) => {
  ...
  console.log(type);
  const resultJokes = jokes.filter((joke) => joke.jokeType == type)
  console.log(resultJokes);
  res.json(resultJokes);
});
//4. POST a new joke
...
app.put("/jokes/:id", (req, res) => {
  ...
  const foundJoke = jokes.find((joke) => joke.id == id);
  foundJoke.jokeText = newText;
  foundJoke.jokeType = newType;
  res.json(foundJoke);
})
...
app.delete("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  if ((jokes.findIndex((joke) => joke.id == id)) != -1) {
    res.json(jokes.splice(id - 1, 1)[0]);
  }
  else
    res
      .status(404)
      .json("We cant find the id");
